...
import {
  ...
  Clock, AlertTriangle, Activity, Globe, Star
} from 'lucide-react'
...
export default async function MasterDashboardPage() {
  ...
  const [
    ...
    prisma.booking.aggregate({
      _sum: { price: true },
      where: { status: { in: ['CONFIRMED', 'COMPLETED', 'PAID'] }, createdAt: { gte: thirtyDaysAgo } },
    }),

    prisma.booking.aggregate({
      _sum: { price: true },
      where: { status: { in: ['CONFIRMED', 'COMPLETED', 'PAID'] }, createdAt: { gte: sevenDaysAgo } },
    }),
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-6">
        ...
        <div className="grid grid-cols-3 lg:grid-cols-6 gap-3">
          ...
          {bookingsByStatus.length === 0 && (
            <div className="col-span-6 text-center text-slate-400 text-sm py-4">Nenhum agendamento ainda</div>
          )}
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
                ...
                <div key={t.id} className="px-4 py-3 flex items-center justify-between hover:bg-slate-50 transition-colors">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-slate-400">{t._count.users} usuários · {t._count.bookings} agendamentos</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${st.color}`}>{st.label}</span>
                    <Link href={`/t/${t.slug}/admin`} className="text-xs text-[#1A6335] hover:underline font-medium">Ver →</Link>
                  </div>
                </div>
              )
            })}
        ...
        <section className="space-y-3">
          <h2 className="font-bold text-slate-900 flex items-center gap-2">
            <Star className="w-4 h-4 text-amber-500" /> Top empresas por agendamentos
          </h2>
          <div className="bg-white rounded-2xl border border-slate-100 divide-y divide-slate-50 overflow-hidden">
            {topTenants.map((t, i) => (
            ...
            {topTenants.length === 0 && (
              <div className="py-10 text-center text-slate-400 text-sm">Nenhum dado ainda</div>
            )}
          </div>
        </section>
      </div>
            ...
            {recentUsers.length === 0 ? (
              ...
              <div key={u.id} className="px-4 py-3 flex items-center justify-between">
                ...
                <div className="text-right shrink-0">
                  ...
                  <p className="text-xs text-slate-300">{new Date(u.createdAt).toLocaleDateString('pt-BR')}</p>
                </div>
              </div>
            ))}
          </div>
